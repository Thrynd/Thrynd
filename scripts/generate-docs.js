 ...
 * 1. Machine-readable JSON for LLM consumption (api/components.json)
 * 2. Optionally syncs to Docs site (../Docs/public/api/components.json)
 *
...
const OUTPUT_FILE = path.join(ROOT_DIR, 'api', 'components.json');
const DOCS_OUTPUT = path.join(ROOT_DIR, '..', 'Docs', 'public', 'api', 'components.json');
...
function main() {
  ...
  // Write to api/components.json
  fs.mkdirSync(path.dirname(OUTPUT_FILE), { recursive: true });
  fs.writeFileSync(OUTPUT_FILE, jsonOutput);
  console.log(`\nWritten: ${OUTPUT_FILE}`);

  // Also write to Docs site if it exists
  if (fs.existsSync(path.dirname(DOCS_OUTPUT))) {
    fs.writeFileSync(DOCS_OUTPUT, jsonOutput);
    console.log(`Written: ${DOCS_OUTPUT}`);
  }
